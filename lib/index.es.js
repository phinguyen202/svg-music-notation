...
var RootSVGMusicNotation = /** @class */ (function (_super) {
    ...
    RootSVGMusicNotation.prototype.render = function () {
        return (createElement("svg", { ref: this.myRef, height: this.props.height, width: this.props.width }, this.state.dimension && this.renderStaves(this.props.staves)));
    };
    /**
    ...
    RootSVGMusicNotation.prototype.renderStaves = function (staves) {
        ...
        var stavesList = staves.map(function (stave, index) {
            if (!index) {
                return (createElement(Stave, { clef: _this.props.clef, measures: stave.measures, width: _this.state.dimension.width, x: 0, y: svgPaddingTop, key: index }));
            }
            return (createElement(Stave, { measures: stave.measures, x: 0, width: _this.state.dimension.width, y: svgPaddingTop + (120 * index), key: index }));
        });
        // rendering padding bottom
        // 97 is stave height wich is calculated from 0 of stave
        stavesList.push(createElement("rect", { y: svgPaddingTop + 97 + (120 * (staves.length - 1)), width: this.state.dimension.width, key: staves.length, height: svgPaddingBottom, fillOpacity: 0, strokeOpacity: 0 }));
        return stavesList;
    };
...
export default SVGMusicNotation;
